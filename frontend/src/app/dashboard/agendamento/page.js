        ...
        <div className="bg-[#0f1319] border border-[#1e2733] rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[#1e2733]">
            <h3 className="font-display font-bold text-sm">
              {selected} de {MONTHS[month - 1]} — {selectedCamps.length}{" "}
              campanha(s)
            </h3>
            <Link
              href="/dashboard/campanhas/nova"
                ...
                return (
                  <div key={c.id} className="flex items-center gap-3 px-4 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-3 mt-0.5 flex-wrap">
                        ...
                        <span className="text-xs text-slate-500">
                          📩 {(c.contacts_total || 0).toLocaleString("pt-BR")}{" "}
                          contatos
                        </span>
                    ...
                    <div className="flex gap-1 flex-shrink-0">
                      {c.status === "scheduled" && (
                        <button
                          onClick={() => campaignsApi.start(c.id).then(load)}
                      ...
                      <Link
                        href={`/dashboard/campanhas/${c.id}/relatorio`}
      ...
      {!selected && (
        <div className="bg-[#0f1319] border border-[#1e2733] rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-[#1e2733]">
            <h3 className="font-display font-bold text-sm">
              Todas as campanhas de {MONTHS[month - 1]}
            </h3>
          </div>
          {loading ? (
            <div className="py-8 text-center text-slate-500 text-sm">
            ...
            <div className="py-10 text-center">
              <p className="text-slate-500 text-sm mb-3">
                Nenhuma campanha agendada neste mês
              </p>
              <Link
                href="/dashboard/campanhas/nova"
              ...
              {Object.entries(calendar)
                ...
                .map(({ day, camp, key }) => {
                  ...
                  const time = camp.scheduled_at
                    ? new Date(camp.scheduled_at).toLocaleTimeString("pt-BR", {
                        hour: "2-digit",
                        minute: "2-digit",
                      })
                    : "—";
                  return (
                    <div
                      key={key}
                      className="flex items-center gap-3 px-4 py-2.5 hover:bg-[#161b24] transition-colors"
                    >
                      ...
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full font-mono ${sc.bg} ${sc.text}`}
                      >
                        {camp.status}
                      </span>
                    </div>
