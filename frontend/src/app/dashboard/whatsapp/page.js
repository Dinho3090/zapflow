    ...
    <div className="max-w-md mx-auto space-y-4">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-0.5">
          Conecte seu número para disparar campanhas
        </p>
      </div>

      {/* Status card */}
      <div
        className={`rounded-xl border p-5 ${isConnected ? "bg-[#00e5a0]/05 border-[#00e5a0]/30" : "bg-[#0f1319] border-[#1e2733]"}`}
      >
        <div className="flex items-center gap-3 mb-4">
          <div
            className={`w-3 h-3 rounded-full ${isConnected ? "bg-[#00e5a0] animate-pulse-dot" : "bg-slate-600"}`}
          />
          <span
            className={`font-semibold text-sm ${isConnected ? "text-[#00e5a0]" : "text-slate-400"}`}
          >
            {loading
              ? "Verificando..."
              : isConnected
      ...
      {qr && !isConnected && (
        <div className="bg-[#0f1319] border border-[#1e2733] rounded-xl p-6 text-center">
          ...
          <div className="text-xs text-slate-500 space-y-1">
            ...
            <p>
              3. Toque em{" "}
              <strong className="text-slate-300">Vincular dispositivo</strong>
            </p>
            <p>4. Aponte a câmera para o QR Code acima</p>
          </div>
          <button
            onClick={connect}
            className="mt-4 text-xs text-[#00e5a0] hover:underline"
          >
            🔄 Gerar novo QR
          </button>
        </div>
      ...
      <div className="bg-amber-500/05 border border-amber-500/20 rounded-xl p-4">
        <p className="text-xs text-amber-300/80">
          <strong>⚠️ Importante:</strong> Use um número dedicado para disparos.
          Evite usar seu número pessoal principal. Respeite os limites de envio
          para reduzir o risco de banimento.
        </p>
      </div>
    </div>
  );
