...
export default function NovaCampanha() {
  ...
  function removeTime(t) {
    set(
      "recurrence_times",
      form.recurrence_times.filter(x => x !== t)
    );
  }
  ...
  async function handleSubmit(e) {
    ...
    if (!form.message_text && !form.media_url)
      return setError("Adicione texto ou mídia");
    if (form.media_url && !form.media_caption)
      return setError("Legenda obrigatória para imagem/vídeo");
    ...
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-slate-400 text-sm mt-1">
          Configure o disparo e o agendamento
        </p>
      </div>
        ...
        <Section title="Conteúdo da Mensagem">
          ...
          {form.media_type !== "none" && (
            <>
              ...
              <Field label="Legenda *">
                <Input
                  placeholder="Legenda obrigatória — aparece abaixo da mídia"
        ...
        <Section title="Comportamento Humano">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Início do envio (hora)">
              <Input
                type="time"
                value={`${String(form.send_start_hour).padStart(2, "0")}:00`}
                onChange={e =>
                  set("send_start_hour", +e.target.value.split(":")[0])
                }
              />
            </Field>
            <Field label="Fim do envio (hora)">
              <Input
                type="time"
                value={`${String(form.send_end_hour).padStart(2, "0")}:00`}
                onChange={e =>
                  set("send_end_hour", +e.target.value.split(":")[0])
                }
              />
            </Field>
          </div>
          <label className="flex items-center gap-2.5 cursor-pointer">
            <div
              onClick={() => set("typing_simulation", !form.typing_simulation)}
              className={`w-9 h-5 rounded-full transition-colors relative ${form.typing_simulation ? "bg-[#00e5a0]" : "bg-[#2a3444]"}`}
            >
              <span
                className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform ${form.typing_simulation ? "translate-x-4" : "translate-x-0.5"}`}
              />
            </div>
          ...
          <label className="flex items-center gap-2.5 cursor-pointer">
            <div
              onClick={() => set("send_on_weekends", !form.send_on_weekends)}
              className={`w-9 h-5 rounded-full transition-colors relative ${form.send_on_weekends ? "bg-[#00e5a0]" : "bg-[#2a3444]"}`}
            >
              <span
                className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform ${form.send_on_weekends ? "translate-x-4" : "translate-x-0.5"}`}
              />
            </div>
            <span className="text-sm text-slate-300">
              Enviar nos finais de semana
            </span>
          </label>
        </Section>
            ...
            <>
              ...
              <Field label="Horários de envio">
                ...
                <p className="text-xs text-slate-500 mt-1.5">
                  {form.recurrence_times.length} horário(s) ={" "}
                  {form.recurrence_times.length} campanha(s) por dia ativo
                </p>
              </Field>
