...
const NODE_COLORS = {
  message: {
    badge: "bg-[#00e5a0]/10 text-[#00e5a0] border-[#00e5a0]/30",
    icon: "💬",
  },
...
function Toggle({ on, onChange }) {
  return (
    <button
      onClick={onChange}
      className={`w-10 h-6 rounded-full transition-colors relative flex-shrink-0 ${on ? "bg-[#00e5a0]" : "bg-[#2a3444]"}`}
    >
      <span
        className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${on ? "translate-x-5" : "translate-x-1"}`}
      />
    </button>
  );
}
    ...
    <div className="relative">
      <div className="flex gap-2.5 p-3 bg-[#161b24] border border-[#2a3444] rounded-xl">
        ...
        <div className="flex flex-col items-center gap-1 flex-shrink-0">
          <span
            className={`text-[10px] font-bold px-2 py-0.5 rounded-full border font-mono ${nc.badge}`}
          >
            {nc.icon} {node.type}
          </span>
            ...
            <div className="text-xs text-slate-500 bg-[#0f1319] rounded-lg px-2.5 py-2 space-y-1 border border-[#1e2733]">
              ...
              {(node.options || []).map((opt, oi) => (
                <div key={oi} className="flex gap-1.5 items-center">
                  <span className="font-mono font-bold text-[#00e5a0] w-4">
                    {opt.key}
                  </span>
                  <input
                    value={opt.label}
                  ...
                  <button
                    onClick={() => {
                      const opts = (node.options || []).filter(
                        (_, i) => i !== oi
                      );
                      onChange({ ...node, options: opts });
                    }}
              ...
              <button
                onClick={() => {
                  ...
                  const opts = [
                    ...(node.options || []),
                    { key: next, label: "", next_node_order: index + 1 },
                  ];
                  onChange({ ...node, options: opts });
                }}
                className="text-[#00e5a0] text-[10px] hover:underline mt-1"
              >
                + Adicionar opção
              </button>
            </div>
...
function AutomationModal({ open, onClose, onSave, initial }) {
  ...
  const [keywords, setKeywords] = useState(
    (initial?.trigger_keywords || []).join(", ")
  );
  const [nodes, setNodes] = useState(
    initial?.automation_nodes || [
      {
        order: 0,
        type: "message",
  ...
  function updateNode(i, n) {
    setNodes(ns => ns.map((x, j) => (j === i ? n : x)));
  }
  function addNode() {
    setNodes(ns => [
      ...ns,
      { order: ns.length, type: "message", content: "", options: null },
    ]);
  }
  function removeNode(i) {
    setNodes(ns =>
      ns.filter((_, j) => j !== i).map((n, j) => ({ ...n, order: j }))
    );
  }
  ...
  return (
    <div
      className="fixed inset-0 bg-black/75 z-[500] flex items-end sm:items-center justify-center p-0 sm:p-4"
    ...
    >
      <div className="bg-[#0f1319] border border-[#1e2733] rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[92vh] overflow-y-auto">
          ...
          {trigger === "keyword" && (
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                O bot responde quando o usuário digitar qualquer uma dessas
                palavras
              </p>
            </div>
          )}
        ...
        <div className="sticky bottom-0 bg-[#0f1319] px-5 py-4 border-t border-[#1e2733] flex justify-end gap-2">
          <button
            onClick={onClose}
            className="px-4 py-2 text-sm text-slate-400 border border-[#2a3444] rounded-lg hover:text-slate-200 transition-colors"
          >
            Cancelar
          </button>
          <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {automations.map(auto => {
            ...
            return (
              <div
                key={auto.id}
                className="bg-[#0f1319] border border-[#1e2733] rounded-xl overflow-hidden"
              >
                  ...
                  {nodes.slice(0, 4).map((n, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-start gap-2">
                        <span
                          className={`text-[10px] px-1.5 py-0.5 rounded-full border font-mono flex-shrink-0 mt-0.5 ${nc.badge}`}
                        >
                          {nc.icon} {n.type}
                        </span>
